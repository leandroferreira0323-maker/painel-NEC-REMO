...
def login(page):
    ...
    try:
        ...
        print(f"ERRO ao tentar logar: {e}", file=sys.stderr)
        try:
            page.screenshot(path="debug_login.png", full_page=True)
            with open("debug_login.html", "w", encoding="utf-8") as f:
                f.write(page.content())
            print(f"URL no momento do erro: {page.url}", file=sys.stderr)
            print("Screenshot salvo em debug_login.png e HTML em debug_login.html", file=sys.stderr)
        except Exception as e2:
            print(f"Não foi possível capturar diagnóstico: {e2}", file=sys.stderr)
        sys.exit(1)
